...
while (True):
    ...
    if os.path.exists("olddata.csv"):
        ...
        response1 = requests.get(url="https://api.github.com/search/repositories?q=CVE2020&sort=updated&per_page=10",
        ...
        datas = []
        requests.packages.urllib3.disable_warnings()
        response1 = requests.get(url="https://api.github.com/search/repositories?q=CVE2020&sort=updated&order=desc",
                                 headers=headers1, verify=False)
        response2 = requests.get(url="https://api.github.com/search/repositories?q=RCE&ssort=updated&order=desc",
                                 headers=headers1, verify=False)
    ...
    for j in [data1["items"], data2["items"]]:
        for i in j:
            ...
            if s1 not in datas:
                ...
                # print(dates)
                params = {
                    "text": s["name"], "desp": "链接:" + str(s["html"]) + "\n简介" + str(s["description"])
                }
                print("当前推送为" + str(s) + "\n")
                print(params)
                requests.packages.urllib3.disable_warnings()
                requests.get("https://sctapi.ftqq.com/SCT167915TblQQ51GRCdlmIVQAYnpyBglj.send?title=messagetitle&desp=messagecontent")
                time.sleep(1)  # 以防推送太猛
                print("推送完成\n")
                datas.append(s1)
            else:
